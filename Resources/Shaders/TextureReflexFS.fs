...
void main()
{
    ...
    vec3 c_reflexie = vec3(textureCube(u_cube_texture, dirReflect));
    // texture color
    vec3 c_proprie  = vec3(texture2D(u_texture,v_uv));
    
    // get the texture
    vec3 objColor = c_reflexie * 0.5 + c_proprie * 0.5; // object color
    // lighting
